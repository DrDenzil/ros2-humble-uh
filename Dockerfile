...
# Enable Ubuntu repos commonly needed for ROS dependencies
...
# Install gazebo + ros-gz (and tools we use below)
...
# Use bash so `source` works in RUN layers
SHELL ["/bin/bash", "-lc"]

# Create workspace + import sources as ros
USER ros
WORKDIR /home/ros
...
# Install deps as root (avoid rosdep trying to sudo)
USER root
USER root
RUN apt-get update \
 && rosdep update \
 && rosdep install --rosdistro humble --from-paths /home/ros/tiago_public_ws/src -y --ignore-src \
 && rm -rf /var/lib/apt/lists/*
